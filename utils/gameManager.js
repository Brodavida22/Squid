...
async function startGame(sock, from, gameType, totalRounds) {
    ...
    let gameTitle = gameType === 'trivia' ? '🎯 TRIVIA SHOWDOWN' : gameType === 'quiz' ? '🧠 QUIZ CHALLENGE' : gameType === 'scramble' ? '🔤 WORD SCRAMBLE' : gameType === 'riddle' ? '🧩 RIDDLE MASTER' : '🔢 NUMBER GUESSING';
    const roundDuration = (gameType === 'trivia' || gameType === 'quiz' || gameType === 'riddle') ? '25s' : '45s';
...
async function nextRound(from) {
    ...
    let roundText = '';
    let roundTimeLimit = (session.gameType === 'trivia' || session.gameType === 'quiz' || session.gameType === 'riddle') ? 25 : 45;
    const diffTag = qData.difficulty ? qData.difficulty.toUpperCase() : 'MEDIUM';
    const diffEmoji = diffTag === 'EASY' ? '🟢' : diffTag === 'HARD' ? '🔴' : '🟡';

    if (session.gameType === 'trivia' || session.gameType === 'quiz' || session.gameType === 'riddle') {
        const icon = session.gameType === 'trivia' ? '🎯 *TRIVIA*' : session.gameType === 'quiz' ? '🧠 *QUIZ*' : '🧩 *RIDDLE*';
        roundText = 
`┏━━━ ${icon} (Round ${session.currentRound}/${session.rounds}) ${diffEmoji} *${diffTag}* ━━━┓\n` +
...
`┗━━━ 👑 *QUEEN VIDA-MD* 👑 ━━━┛`;
        ...
        roundText = 
`┏━━━ 🔤 *WORD SCRAMBLE* (Round ${session.currentRound}/${session.rounds}) ${diffEmoji} *${diffTag}* ━━━┓\n` +
`┃ 🔀 *Scrambled:* \`${scrambled}\`\n` +
`┃ 💡 *Hint:* ${qData.hint}\n` +
`┣━━━━━━━━━━━━━━━━━━━━━━━\n` +
...
`┗━━━ 👑 *QUEEN VIDA-MD* 👑 ━━━┛`;
        ...
        roundText = 
`┏━━━ 🔢 *NUMBER GUESS* (Round ${session.currentRound}/${session.rounds}) ${diffEmoji} *${diffTag}* ━━━┓\n` +
...
`┗━━━ 👑 *QUEEN VIDA-MD* 👑 ━━━┛`;
    ...
    session.timer = setTimeout(() => {
        ...
        if (session.gameType === 'trivia' || session.gameType === 'quiz' || session.gameType === 'riddle') timeOutText += `📌 *Correct Answer was:* *${qData.answer}* (${qData.options[qData.answer]})`;
